...
$sql="select * from cases where status='1' and take='1' and judge_id='".$_SESSION['JUDGE_ID']."' order by id desc";
                ...
                <div class = "item">
                    ...
                    <div class = "item-detail">
                        ...
                        <div class = "item-price">
                            <?php
                                $id=$row['id'];
                                $sqli="select * from appoint where cases_id='$id' and status='1'";
                                $appoint_res=mysqli_query($con,$sqli);
                                if(mysqli_num_rows($appoint_res)>0){
                                    while($appoint_row=mysqli_fetch_assoc($appoint_res)){
                                ?>
                                <span href = "#" class = "item-name">Appointment :  <?php echo $appoint_row['appoint']?></span>
                                <?php } }?>
                        </div>

                        <?php if($row['hearing']=='1'){ ?>
                            <div>
                                <?php
                                    $id=$row['id'];
                                    $sqli="select * from hearing where cases_id='$id'";
                                    $reappoint_res=mysqli_query($con,$sqli);
                                    if(mysqli_num_rows($appoint_res)>0){
                                        while($reappoint_row=mysqli_fetch_assoc($reappoint_res)){
                                    ?>
                                    <span href = "#" class = "item-name">Reappointment :  <?php echo $reappoint_row['reappoint']?></span><br>

                                    <?php if($reappoint_row['reappoint']=='0000-00-00 00:00:00'){ ?>
                                        <a href="manage_court.php?id=<?php echo $reappoint_row['id']?>"><label class="btn btn-success mr-2 hand_cursor">Add Reappointment</label></a>
                                    <?php }else{ ?>
                                        <a href="manage_court.php?id=<?php echo $reappoint_row['id']?>"><label class="btn btn-secondary mr-2 hand_cursor">Edit Reappointment</label></a>&nbsp;
                                    <?php } ?>
                                <?php } }?>
                            </div>
                        <?php } ?>
                        
                        <?php if($row['hearing']=='0'){ ?>
                            <a href="manage_hearings.php?id=<?php echo $row['id']?>"><label class="btn btn-success mr-2 hand_cursor">Start Hearing</label></a>
                        <?php }else{ ?>
                            <?php
                                $id=$row['id'];
                                $sqli="select * from hearing where cases_id='$id'";
                            ...
                            ?>
                                <?php if($reappoint_row['reappoint']=='0000-00-00 00:00:00'){ ?>
                                    <a href="manage_hearings.php?id=<?php echo $row['id']?>"><label class="btn btn-secondary mr-1 hand_cursor">Edit Hearing</label></a>&nbsp;
                                <?php }else{ ?>
                                    <a href="manage_hearings.php?id=<?php echo $row['id']?>"><label class="btn btn-success mr-2 hand_cursor">Start Hearing</label></a>
                                <?php } ?>
                                    <a href="manage_conclution.php?id=<?php echo $row['id']?>"><label class="btn btn-warning mr-1 hand_cursor">Add Conclution</label></a>
                            <?php } }?>  
